...
getCoords().then(coords => {
    ...
    marker = new H.map.DomMarker({ lat: coords.lat, lng: coords.long },{icon: icon})
...
oReq.onload = function() {
    ...
    for (let i = 0; i < Math.floor(toilets.length); i++) {
        ...
        marker = new H.map.DomMarker({ lat: toilet.coordinates[0], lng: toilet.coordinates[1] },{icon: iconsvg})
        map.addObject(marker);
